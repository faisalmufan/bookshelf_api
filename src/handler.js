...
const getAllBookHandler = (request) => {
  ...
  if (name && books.filter((n) => n.name.toLowerCase().indexOf(name.toLowerCase()) > -1)) {
    const withName = books.filter((n) => n.name.toLowerCase().indexOf(name.toLowerCase()) > -1);
    ...
    const withReading = books.filter((r) => r.reading === (parseInt(reading) === 1));
    ...
    const withFinished = books.filter((r) => r.finished === (parseInt(finished) === 1));
